...
#include "Abilities/AbilityTasks/MPRAT_WaitTargetDataUsingActor.h"
#include "AbilitySystemComponent.h"
#include "Abilities/TargetActors/MPRGATargetActor_Trace.h"

UMPRAT_WaitTargetDataUsingActor::UMPRAT_WaitTargetDataUsingActor(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{

}

UMPRAT_WaitTargetDataUsingActor* UMPRAT_WaitTargetDataUsingActor::WaitTargetDataWithReusableActor(UGameplayAbility* OwningAbility, FName TaskInstanceName, TEnumAsByte<EGameplayTargetingConfirmation::Type> ConfirmationType, AGameplayAbilityTargetActor* InTargetActor, bool bCreateKeyIfNotValidForMorePredicting)
{
	UMPRAT_WaitTargetDataUsingActor* MyObj = NewAbilityTask<UMPRAT_WaitTargetDataUsingActor>(OwningAbility, TaskInstanceName);		//Register for task list here, providing a given FName as a key
...
void UMPRAT_WaitTargetDataUsingActor::Activate()
{
	if (IsPendingKill())
	{
		return;
...
void UMPRAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback(const FGameplayAbilityTargetDataHandle& Data, FGameplayTag ActivationTag)
{
...
void UMPRAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback()
{
	check(AbilitySystemComponent);
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UMPRAT_WaitTargetDataUsingActor::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent);
	if (!Ability)
	{
...
void UMPRAT_WaitTargetDataUsingActor::OnTargetDataCancelledCallback(const FGameplayAbilityTargetDataHandle& Data)
{
	check(AbilitySystemComponent);
...
void UMPRAT_WaitTargetDataUsingActor::ExternalConfirm(bool bEndTask)
{
	check(AbilitySystemComponent);
	if (TargetActor)
	{
...
void UMPRAT_WaitTargetDataUsingActor::ExternalCancel()
{
	check(AbilitySystemComponent);
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
void UMPRAT_WaitTargetDataUsingActor::InitializeTargetActor() const
{
	check(TargetActor);
	check(Ability);
	...
	TargetActor->TargetDataReadyDelegate.AddUObject(const_cast<UMPRAT_WaitTargetDataUsingActor*>(this), &UMPRAT_WaitTargetDataUsingActor::OnTargetDataReadyCallback);
	TargetActor->CanceledDelegate.AddUObject(const_cast<UMPRAT_WaitTargetDataUsingActor*>(this), &UMPRAT_WaitTargetDataUsingActor::OnTargetDataCancelledCallback);
}

void UMPRAT_WaitTargetDataUsingActor::FinalizeTargetActor() const
{
	check(TargetActor);
	check(Ability);
...
void UMPRAT_WaitTargetDataUsingActor::RegisterTargetDataCallbacks()
{
	...
	if (!bIsLocallyControlled)
	{
		...
		if (!bShouldProduceTargetDataOnServer)
		{
			...
			AbilitySystemComponent->AbilityTargetDataSetDelegate(SpecHandle, ActivationPredictionKey).AddUObject(this, &UMPRAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCallback);
			AbilitySystemComponent->AbilityTargetDataCancelledDelegate(SpecHandle, ActivationPredictionKey).AddUObject(this, &UMPRAT_WaitTargetDataUsingActor::OnTargetDataReplicatedCancelledCallback);
...
void UMPRAT_WaitTargetDataUsingActor::OnDestroy(bool AbilityEnded)
{
	if (TargetActor)
	{
		AMPRGATargetActor_Trace* TraceTargetActor = Cast<AMPRGATargetActor_Trace>(TargetActor);
		if (TraceTargetActor)
		{
...
bool UMPRAT_WaitTargetDataUsingActor::ShouldReplicateDataToServer() const
{
	if (!Ability || !TargetActor)
	{
		return false;
