...
AMPRGATargetActor_Trace::AMPRGATargetActor_Trace()
{
...
void AMPRGATargetActor_Trace::ResetSpread()
{
	bUseAimingSpreadMod = false;
	BaseSpread = 0.0f;
	AimingSpreadMod = 0.0f;
...
float AMPRGATargetActor_Trace::GetCurrentSpread() const
{
...
void AMPRGATargetActor_Trace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void AMPRGATargetActor_Trace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void AMPRGATargetActor_Trace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	bDestroyOnConfirmation = bInDestroyOnConfirmation;
}

void AMPRGATargetActor_Trace::StartTargeting(UGameplayAbility* Ability)
{
...
void AMPRGATargetActor_Trace::ConfirmTargetingAndContinue()
{
...
void AMPRGATargetActor_Trace::CancelTargeting()
{
...
void AMPRGATargetActor_Trace::BeginPlay()
{
	Super::BeginPlay();
...
void AMPRGATargetActor_Trace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DestroyReticleActors();
...
void AMPRGATargetActor_Trace::Tick(float DeltaSeconds)
{
...
void AMPRGATargetActor_Trace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World, const FGameplayTargetDataFilterHandle FilterHandle, const FVector& Start, const FVector& End, FName ProfileName, const FCollisionQueryParams Params)
{
	check(World);
...
void AMPRGATargetActor_Trace::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params, const FVector& TraceStart, FVector& OutTraceEnd, bool bIgnorePitch)
{
...
bool AMPRGATargetActor_Trace::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter, float AbilityRange, FVector& ClippedPosition)
{
...
void AMPRGATargetActor_Trace::StopTargeting()
{
	SetActorTickEnabled(false);
...
FGameplayAbilityTargetDataHandle AMPRGATargetActor_Trace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
...
TArray<FHitResult> AMPRGATargetActor_Trace::PerformTrace(AActor* InSourceActor)
{
	check(InSourceActor);
	...
	FCollisionQueryParams Params(SCENE_QUERY_STAT(AMPRGATargetActor_LineTrace), bTraceComplex);
...
AGameplayAbilityWorldReticle* AMPRGATargetActor_Trace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void AMPRGATargetActor_Trace::DestroyReticleActors()
{
